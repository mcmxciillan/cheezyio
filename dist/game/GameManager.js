...
class GameManager {
    ...
    cheese = {};
    powerups = {};
    deadPlayers = {}; // Persist dead stats
    loopInterval = null;
    // Game Constants
    WORLD_SIZE = 2000;
    MAX_CHEESE = 100;
    ...
    // Spatial Hashing
    CELL_SIZE = 1000; // 1000x1000 cells (Large enough for AOI Viewport)
    grid = new Map();
    getCellKey(x, y) {
    ...
    replenishCheese() {
        ...
        if (currentCheese < this.MAX_CHEESE) {
            ...
            for (let i = 0; i < needed; i++) {
                ...
                const types = ['crumbs', 'wedge', 'block', 'wheel'];
    ...
    replenishPowerUps() {
        ...
        if (current < MAX_POWERUPS && Math.random() < 0.05) { // Slow spawn chance per tick
            const id = Math.random().toString(36).substr(2, 9);
            const types = ['speed', 'ghost', 'magnet', 'ninja'];
    ...
    addPlayer(id, name, startScore = 0) {
    ...
    updateBots() {
        Object.values(this.players).forEach(p => {
            ...
            nearbyPlayers.forEach(other => {
                ...
                if (other.activeEffects?.ghost && other.activeEffects.ghost > Date.now())
                    return;
            ...
            for (const p2 of nearby) {
                ...
                for (const h1 of hitboxes1) {
                    for (const h2 of hitboxes2) {
                        if (h1.isClone && !p1.ninjaState?.activeClones[h1.index])
                            continue;
                        if (h2.isClone && !p2.ninjaState?.activeClones[h2.index])
                            continue;
                        if (h1.radius <= h2.radius * 1.1)
                            continue; // h1 must be bigger
                        const dx = h1.x - h2.x;
                        const dy = h1.y - h2.y;
                        const dist = Math.sqrt(dx * dx + dy * dy);
                        if (dist < h1.radius) {
    ...
    getPlayerHitboxes(p) {
        ...
        if (p.activeEffects?.ninja && p.activeEffects.ninja > Date.now() && p.ninjaState) {
            ...
            [0, 1, 2].forEach(idx => {
                if (idx === p.ninjaState.formationIdx)
                    return; // Skip Real Player Position (already added)
                if (!p.ninjaState.activeClones[idx])
                    return; // Skip popped clones
    ...
    broadcastState() {
        ...
        const compressedCheese = {};
        const allPlayerIds = []; // All players (human and bot) who will receive a direct AOI update
        Object.values(this.players).forEach(p => {
        ...
        Object.values(this.players).forEach(p => {
            if (p.isBot)
                return; // Bots don't need updates
    ...
    spawnCheeseExplosion(x, y, score) {
        let remaining = score;
